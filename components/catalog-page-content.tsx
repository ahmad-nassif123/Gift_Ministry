...
import { catalogStorageKey, filterProductsByCatalogScope } from "@/lib/catalog-scope";
import {
  loadCatalogProductsFromLocalStorage,
...
function CatalogPageInner({
  ...
  const seedProducts = useMemo(
    () => filterProductsByCatalogScope(initialProducts, scope),
    [scope]
  );
  ...
  useEffect(() => {
    if (scope !== "public" || typeof window === "undefined") return;
    try {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      const data = await fetchCatalogProducts(scope);
      if (data) {
        setAllProducts(data);
        safeLocalStorageSetItem(catalogStorageKey(scope), JSON.stringify(data));
        return;
      }
      setAllProducts(loadCatalogProductsFromLocalStorage(scope));
    };
  ...
  const catalogFilterKey = `${scope}__${selectedGiftTier ?? "all"}__${searchLower}`;
  ...
  return (
    <div className="flex min-h-screen flex-col">
      ...
      <main className="flex-1">
        ...
        <section id={productsSectionId} className="py-6 sm:py-14 md:py-24">
              ...
              <div>
                ...
                >
                  ...
                  {giftTiers.map((tier) => (
                    <Badge
                      key={tier}
                      variant={selectedGiftTier === tier ? "default" : "outline"}
                      className="cursor-pointer text-base px-4 py-2 min-h-[44px] inline-flex items-center transition-all duration-200 hover:shadow-md active:scale-[0.98]"
...
export function CatalogPageContent(props: CatalogPageContentProps) {
  return (
    <Suspense
      fallback={
        <div className="flex min-h-screen flex-col">
